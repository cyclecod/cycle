...
@Component({
  selector: 'ph-danger-discover',
  templateUrl: './discover-danger-list.page.html',
  styleUrls: ['./discover-danger-list.page.scss'],
})
export class DiscoverDangerListComponent implements OnInit, OnChanges {
  state = {
    refreshState: {
  ...
  async getList(query: any) {
    ...
    await this.http.post('getDiscoverNotDoDangerList', query).then(async (res: any) => {
      if (res.code === 200) {
  ...
  */
  ...
  //  我的整改
  checkYH(event) {
    this.router.navigate(['risk-danger-details'], { queryParams: event });
  }
  titleBack(): void {
    history.go(-1);
  }
