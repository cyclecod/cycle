 ...
 * @LastEditors: 九阳
 * @LastEditTime: 2022-10-24 12:27:57
 */
...
@Component({
  selector: 'ph-danger-review',
  templateUrl: './danger-review.component.html',
  styleUrls: ['./danger-review.component.less'],
})
export class DangerReviewComponent implements OnInit, OnDestroy {
  ...
  async saveData(object) {
    ...
    await this.http.post('rectificAtionDanger', object).then((res: any) => {
      if (res.code === 200) {
  ...
  check() : boolean {
    ...
    if (!this.isNull(this.objectReview.cost)) {
      error = error + '整改金额不能是空.';
    }
